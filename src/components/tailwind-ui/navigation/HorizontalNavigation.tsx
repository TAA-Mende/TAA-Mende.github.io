...
function InternalHorizontalNavigation({
  ...
  return (
    <toggleNavigationContext.Provider value={contextValue}>
...
>(props: HorizontalNavigationItemProps<T, U>) {
  ...
  const contextValue = useMemo(
    () => ({ isSelected: props.isSelected, onSelect: props.onSelect }),
    [props.isSelected, props.onSelect],
  );
  return (
    <baseOptionContext.Provider value={contextValue}>
      {renderOption ? (
